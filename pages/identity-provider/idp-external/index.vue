<template>
  <div class="h-full" :class="{ 'h-screen': lists.length <= 10 }">
    ...
    <div class="flex justify-between items-center py-5">
      <div class="text-2xl font-normal">IdP External</div>
      <base-button
        color="primary"
        class="flex items-center font-thin text-sm"
        @click="
          $router.push('/identity-provider/idp-external/new-idp-external')
        "
      >
        <base-icon viewBox="0 0 24 24" size="20" icon="add" />
        Add Identity
      </base-button>
    </div>
    <!-- END TITLE -->
    ...
    <!-- CONTENT -->
    <div class="bg-white w-full rounded-md border border-cool-150 p-4">
      ...
      <div class="table-custom">
        <table>
          <thead>
            ...
            <th class="text-left">Default Scrope</th>
            <th>Create date</th>
            <th>Status</th>
            <th></th>
          </thead>
          <tbody>
            <tr v-for="(item, i) in lists" :key="i">
              ...
              <td>{{item.defualt_scrope}}</td>
              <td class="text-center">{{ $model.formatShortMonth(item.created_at) }}</td>
      ...
      lists: [
        {
          name: "Facebook",
          client_id: "Facebook",
          defualt_scrope: "openid, profile, email",
