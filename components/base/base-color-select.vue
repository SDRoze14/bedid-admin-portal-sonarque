<template>
  <div v-click-outside="hideEvent" class="relative w-full">
    <div class="flex items-center">
      <div class="w-8">
        <div class="w-8 h-8 rounded border border-gray-300 cursor-pointer"
          v-bind:style="`background-color: ${color.hex8}`"
          @click="openClick"  
        ></div>
      </div>
    ...
    <div class="absolute origin-top-right rounded-md shadow-md z-10" v-show="open">
      <sketch-picker :value="color" v-on="inputListeners" v-bind="$attrs"></sketch-picker>
    </div>
  </div>
</template>
